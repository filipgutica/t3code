...
Drag from the button to include more blocks; ordinary text selection keeps its native behavior.
Selected blocks stay tinted while you add your comment below the context.

It appears above the composer and is sent with your next message, not as a separate reply thread.
You can remove it before sending. Links, code actions, task checkboxes, and other response controls
remain available.
